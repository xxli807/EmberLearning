...
namespace ConsumerPortal.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public JsonResult JsonLogin(LoginModel model, string returnUrl)
        {
            ...
            {
                var name = WebSecurityExtension.GetUserName(model.email);
                if (!string.IsNullOrEmpty(name) && WebSecurity.Login(name, model.password, persistCookie: true))
                {
        ...
        public ActionResult JsonRegister(RegisterModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    InitiateDatabaseForNewUser(model.name);

                    FormsAuthentication.SetAuthCookie(model.name, createPersistentCookie: false);
                    return Json(new { success = true, redirect = returnUrl });
                }
        ...
        public ActionResult ExternalLoginConfirmation(RegisterExternalLoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    UserProfile user = db.UserProfiles.FirstOrDefault(u => u.Name.ToLower() == model.UserName.ToLower());
                    // Check if user already exists
                    if (user == null)
                    {
                        // Insert name into the profile table
                        db.UserProfiles.Add(new UserProfile { Name = model.UserName });
                        db.SaveChanges();
                    ...
                    {
                        ModelState.AddModelError("Name", "User name already exists. Please enter a different user name.");
                    }
                }
            }
